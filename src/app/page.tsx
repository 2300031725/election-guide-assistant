...
import { ArrowRight, Bot, Calendar, FileText, CheckCircle2 } from "lucide-react";
...
export default function Home() {
  return (
    <div className="flex-1 flex flex-col items-center justify-center relative overflow-hidden bg-gradient-to-b from-brand-50 to-background w-full">
      ...
      <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] rounded-full bg-brand-200/40 blur-3xl mix-blend-multiply opacity-50 z-0"></div>
      <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] rounded-full bg-accent-200/40 blur-3xl mix-blend-multiply opacity-50 z-0"></div>

      <div className="container px-4 py-24 mx-auto relative z-10 flex flex-col items-center text-center">
        <div className="inline-flex items-center rounded-full border border-brand-200 bg-brand-50 px-3 py-1 text-sm text-brand-600 mb-8">
        ...
        <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight text-foreground mb-6 max-w-4xl">
          Understand Elections. <br className="hidden md:block"/>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-20 w-full justify-center max-w-md mx-auto">
          <Link href="/chat" className="flex-1 inline-flex items-center justify-center gap-2 rounded-xl bg-brand-600 px-8 py-4 text-white font-semibold shadow-lg shadow-brand-600/30 hover:bg-brand-700 transition-all hover:-translate-y-1">
            <Bot size={20} />
            Ask Assistant
          </Link>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-5xl">
          <FeatureCard 
            icon={<CheckCircle2 className="text-accent-500" size={32} />}
            title="Step-by-Step Guide"
            description="From voter registration to election day, know exactly what to do."
          />
          <FeatureCard 
            icon={<Bot className="text-brand-500" size={32} />}
            title="Smart Chatbot"
...
function FeatureCard({ icon, title, description }: { icon: React.ReactNode, title: string, description: string }) {
  return (
    <div className="flex flex-col items-center text-center p-8 rounded-2xl bg-card border border-border shadow-sm hover:shadow-md transition-shadow">
